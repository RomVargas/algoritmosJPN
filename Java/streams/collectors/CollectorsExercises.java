...
public class CollectorsExercises {
    ...
    static List<Review> getReviews(Stream<Videogame> stream) {
        List<Review> r = stream.flatMap(
                v -> v.getReviews().stream())
    ...
    static Map<Boolean, List<Videogame>> getWebSites(Stream<Videogame> stream) {
        Map<Boolean, List<Videogame>> r = stream.collect(Collectors.partitioningBy(
                v -> v.getOfficialWebsite().length() < 15
        )) ;
        return r;
    }
    ...
    static Map<Console, Double> getConsolesPricesAvg(Stream<Videogame> stream) {
        Map<Console, Double> r  = stream.collect(Collectors.groupingBy(Videogame::getConsole, Collectors.averagingDouble(Videogame::getTotalSold)));
        return r;
    }
}
